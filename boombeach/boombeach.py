...
class BoomBeach:
    ...
    def __init__(self, bot):
        ...
        self.queuechannel = "340269179737210890"
        self.queue = dataIO.load_json(queue_file)
    ...
    async def queue(self, ctx):
        ...
        '''    print("queue function")
            ...
            queuemessage = "The following {0}queue subcommands are available:\n\n**add** - adds the specified TF to the queue\n**remove** - removes the specified TF from the queue\n**ack** - acknowledges a ping to put a recruitment post up\n**violation** - adds a violation to the specified TF\n**post** - posts the current queue\n**rules** - posts the recruitment queue rules\n\nThis message and your `{0}queue` message will be deleted after 60 seconds.".format(ctx.prefix)
            ...
            sentmessage = await self.bot.send_message(self.queuechannel, queuemessage)
            await asyncio.sleep(60)
            await self.bot.delete_message(ctx.message)
            await self.bot.delete_message(sentmessage)
            
            # I will also need something to modify the posting date in the queue - if there's a situation where the date needs to be changed
            #   ex. If there's only 1 TF in the queue, and they want to be moved back a day or two
            #
            # Also - I might need a queue manipulation function. If 2 TFs want to swap, can use it to push a TF up or down the queue.
            #
            # Might also need a queue list function
            #
            # Maybe add a thing to add/remove pinged members from the json.
            #
            # Also ***IMPORTANT*** - have BC/Smoke added to the queue as 1 TF instead of 2
            #
    ...
    @queue.command(no_pm=True, pass_context=True, name="add")
    async def queue_add(self, ctx, *, tf):
        ...
        if tf is None:
            ...
            await self._delnewmembermsgs(ctx.message)
            await self._delnewmembermsgs(addmessage)
        else:
            if tf.lower() in qf["TFs"].keys().lower():
                # check to see if the tf is already on the queue
                if self.queue["queue"] is not None:
                ...
                if admin is True or tf.lower() in userroles.lower():
                    position = len(self.queue["queue"]) + 1
                    self.queue["queue"][str(position)]["position"] = position
                    self.queue["queue"][str(position)]["TF"] = tf
                    self.queue["queue"][str(position)]["addedby"] = ctx.message.author.id
                    self.queue["queue"][str(position)]["ack"] = False
                    self.queue["queue"][str(position)]["ackpost"] = None
                    added = datetime.utcnow()
                    self.queue["queue"][str(position)]["added"] = added
                    ...
                    if position is 1:
                        ...
                        self.queue["settings"]["queuebegin"] = tomorrowutc # self.queue["queue"][str(position)]["added"]
                        self.queue["queue"][str(position)]["posttime"] = tomorrowutc
                        self.queue["queue"][str(position)]["pingtime"] = tomorrowutc - timedelta(hours=4) # ping 4 hours before post is to go up
                    else:
                        getpostday = self.queue["queue"][str(position - 1)]["posttime"] + timedelta(days=2)
                        postdayutc = datetime.datetime(getpostday.year, getpostday.month, getpostday.day)
                        self.queue["queue"][str(position)]["posttime"] = postdayutc
                        self.queue["queue"][str(position)]["pingtime"] = postdayutc - timedelta(hours=4) # ping 4 hours before post is to go up
                    dataIO.save_json(queue_file, self.queue)
                    await self._queue_post()
                    addedmsg = await self.bot.say("{} has been added to the queue.".format(tf))
                    await asyncio.sleep(30)
                    await self._delnewmembermsgs(ctx.message)
                    await self._delnewmembermsgs(addedmsg)
                else:
                    ...
                    await asyncio.sleep(30)
                    await self._delnewmembermsgs(ctx.message)
                    await self._delnewmembermsgs(norole)
            else:
                ...
                await asyncio.sleep(30)
                await self._delnewmembermsgs(ctx.message)
                await self._delnewmembermsgs(notvalid)
    
    @queue.command(no_pm=True, pass_context=True, name="remove")
    async def queue_remove(self, ctx, *, tf):
        ...
        if admin is False and tf.lower() not in userroles.lower():
            ...
            asyncio.sleep(30)
            await self._delnewmembermsgs(ctx.message)
            await self._delnewmembermsgs(errormsg)
        
        if tf.lower() not in qf["TFs"].keys().lower():
        ...
        for key in self.queue["queue"]:
            if tf.lower() == self.queue["queue"][key]["TF"].lower():
                removednumber = self.queue["queue"][key]["position"]
                savedtimestamp = self.queue["queue"][key]["added"]
                break
        if removednumber is None:
            ...
            await asyncio.sleep(30)
            await self._delnewmembermsgs(ctx.message)
            await self._delnewmembermsgs(removemsg)
        else:
            count = removednumber
            while count < len(self.queue["queue"]):
                curposttime = self.queue["queue"][str(count)]["posttime"]
                curpingtime = self.queue["queue"][str(count)]["pingtime"]
                self.queue["queue"][str(count)] = self.queue["queue"][str(count+1)]
                self.queue["queue"][str(count)]["position"] = count
                # preserve original post time of previos TF
                self.queue["queue"][str(count)]["posttime"] = curposttime
                self.queue["queue"][str(count)]["pingtime"] = curpingtime
                count += 1
            del self.queue["queue"][str(len(self.queue["queue"]))]
            dataIO.save_json(queue_file, self.queue)
    ...
    @queue.command(no_pm=True, pass_context=True, name="listtfs")
    async def queue_listtfs(self, ctx):
        """Displays a list of valid TFs to add to the queue."""
        tflist = ", ".join(self.queue["TFs"].keys())
        listmessage = self.bot.say("The following TFs/TF families can be added to the queue:\n\n```\n{}\n```\n\nIf there is an issue with the list please contact Annihilator6000 (`@Annihilator6000#2526`).".format(tflist))
        await asyncio.sleep(30)
        await self._delnewmembermsgs(ctx.message)
        self._delnewmembermsgs(listmessage)
    
    @queue.command(no_pm=True, pass_context=True, name="move")
    async def queue_move(self, ctx, *, direction:str=None, tf):
        ...
        if tf.lower() not in self.queue["TFs"].keys().lower():
            ...
            asyncio.sleep(30)
            await self._delnewmembermsgs(ctx.message)
            await self._delnewmembermsgs(notifymsg)
            return
        ...
        for key in self.queue["queue"]:
            if tf.lower() == self.queue["queue"][key]["TF"].lower():
                currentposition = self.queue["queue"][key]["position"]
                break
        ...
        if (currentposition == 0 and direction == "up") or (currentposition == len(self.queue["queue"]) and direction == "down"):
        ...
        tempdata = self.queue["queue"][str(newpos)]
        newposposttime = self.queue["queue"][str(newpos)]["posttime"]
        newpospingtime = self.queue["queue"][str(newpos)]["pingtime"]
        curpostposttime = self.queue["queue"][str(currentposition)]["posttime"]
        curpostpingtime = self.queue["queue"][str(currentposition)]["pingtime"]
        self.queue["queue"][str(newpos)] = self.queue["queue"][str(currentposition)] # 3 to 2
        self.queue["queue"][str(newpos)]["position"] = newpos
        # newposition needs currentposition's post/ping times
        self.queue["queue"][str(newpos)]["posttime"] = curpostposttime
        self.queue["queue"][str(newpos)]["pingtime"] = curpostpingtime
        self.queue["queue"][str(currentposition)] = tempdata # temp to 3
        self.queue["queue"][str(currentposition)]["position"] = currentposition
        # currentposition needs newposition's post/ping times
        self.queue["queue"][str(currentposition)]["posttime"] = newposposttime
        self.queue["queue"][str(currentposition)]["pingtime"] = newpospingtime
        dataIO.save_json(queue_file, self.queue)
        ...
        asyncio.sleep(30)
        await self._delnewmembermsgs(ctx.message)
        await self._delnewmembermsgs(notifymessage)
    ...
    @queue.command(no_pm=True, pass_context=True, name="violation")
    async def queue_violation(self, ctx, *, tf, reason):
        ...
        #
        # add 1 month/3 months to the first/second vios and add that as a unix timestamp to self.queue["violations"][tf]["time"] and increment their violation count
        #
        # "Whisky" : {
        #    "count" : 1,
        #    "time" : null,
        ...
        if admin is False:
            ...
            await self._delnewmembermsgs(notifymsg)
            return
        if tf.lower() not in self.queue["TFs"].keys().lower(): # I don't actually know if I can do keys().lower() ... # might have to do something like: ", ".join(keys()).lower()
            ...
            await self._delnewmembermsgs(notifymsg)
            return
        if tf.lower() in self.queue["violations"].keys().lower(): # probably not going to work.
            if self.queue["violations"][tf]["count"] == 3:
                # already at the max
                ...
                await self._delnewmembermsgs(notifymsg)
                return
            self.queue["violations"][tf]["count"] += 1
            timeout = datetime.utcnow()
            #timeoutfuture = None
            if self.queue["violations"][tf]["count"] == 2:
                timeout += timedelta(days=90)
            if self.queue["violations"][tf]["count"] == 3:
                #indefinate
                timeout = datetime.max()
                self.queue["violations"][tf]["indefinate"] = True
            self.queue["violations"][tf]["time"] = timeout
            self.queue["violations"][tf]["reason"] = reason
        else:
            # TF not in the vio list, add them as a first vio
            self.queue["violations"][tf]["count"] = 1
            self.queue["violations"][tf]["time"] = datetime.utcnow() + timedelta(days=30)
            self.queue["violations"][tf]["indefinate"] = False
            self.queue["violations"][tf]["reason"] = reason
        
        violationtext = { 1 : "for 30 days", 2 : "for 90 days", 3 : "indefinately"}
        viomessage = self.bot.say("{}'s violation has been added with reason \"{}\" and will not be able to request {}.".format(tf, reason, violationtext[self.queue["violations"][tf]["count"]]))
        asyncio.sleep(30)
        await self._delnewmembermsgs(ctx.message)
        await self._delnewmembermsgs(notifymsg)
        
    @queue.command(no_pm=True, pass_context=True, name="post")
    async def queue_post(self, ctx):
        """Posts or updates the current queue. Handy for if it accidentally gets deleted."""
        self._queue_post()
        await self.bot.delete_message(ctx.message)
    
    @queue.command(no_pm=True, pass_context=True, name="rules")
    async def queue_rules(self, ctx):
        ...
        self.queue["settings"]["rulespost"] = str(rulespost.id)
        await self.bot.delete_message(ctx.message)
    ...
    def queue_get(self):
        if len(self.queue["queue"]) < 1:
            ...
            queueobj = []
            count = 1
            length = len(self.queue["queue"])
            if length == 0:
                return ""
            while count <= length:
                tf = self.queue["queue"][str(count)]["TF"]
                if "Bootcamp" in tf or "Smoke" in tf:
                    tf = "BC/Smoke"
                tf = tf + ":"
                posttime = datetime.utcfromtimestamp(self.queue["queue"][str(count)]["posttime"])
    ...
    async def _queue_post(self):
        ...
        if self.queue["settings"]["violations"] is not None:
        ...
        chan = '260933604706615296' #bot-testing channel
        #chan = '232939832849072130' #recruitmentqueue channel
        if self.queue["settings"]["queuepost"] is not None:
            # have the bot edit the post instead of post a new one
            currentmsg = await self.bot.get_message(self.bot.get_channel(chan), self.queue["settings"]["queuepost"])
            ...
            self.queue["settings"]["queuepost"] = str(newqueue.id)
        dataIO.save_json(queue_file, self.queue)
    ...
    async def queue_violatation(self):
        ...
        violist = None
        for tf in self.queue["violations"]:
            if self.queue["violations"][tf]["time"] is not None:
                # valid vio
                viodate = date.fromtimestamp(self.queue["violations"][tf]["time"])
                vio = "{} {} and can not be on the queue until {}".format(tf, self.queue["violations"][tf]["reason"], viodate.ctime().split()[1] + " " + viodate.ctime().split()[2])
                violist.append()
    ...
    async def queue_remove(self, tf):
        ...
        if removednumber is None:
            ...
            count = removednumber
            while count <= len(self.queue["queue"]) and count > 1:
                curposttime = self.queue["queue"][str(count)]["posttime"]
                curpingtime = self.queue["queue"][str(count)]["pingtime"]
                self.queue["queue"][str(count)] = self.queue["queue"][str(count+1)]
                self.queue["queue"][str(count)]["posttime"] = curposttime
    ...
    async def queue_loop(self):
        while self == self.bot.get_cog('BoomBeach'):
            ...
            # }
            now = datetime.utcnow()
            print("queue loop - @ {}, queue length: {}".format(now, len(self.queue["queue"])))
            if len(self.queue["queue"]) > 0:
                # make sure there's something in the queue before trying to access stuff
                if now > self.queue["queue"]["1"]["posttime"] + self.queueduration:
                    # if it's been more than 2 days that the post has been able to be up then delete it, along with the ackpost (ping)
                    if self.queue["queue"]["1"]["ackpost"] is not None:
                        await self._delnewmembermsgs(self.queue["queue"]["1"]["ackpost"])
                    await self.queue_remove(self.queue["queue"]["1"]["TF"])
                if now < self.queue["queue"]["1"]["posttime"] and now > self.queue["queue"]["1"]["pingtime"] and self.queue["queue"]["1"]["ackpost"] is None:
                    # if it's before the time to post, after the time to ping, and a ping hasn't been sent...
                    pingpeople = self.queue["TFs"][self.queue["queue"]["1"]["TF"]]
                    if len(pingpeople) > 2:
                    ...
                    pingmsg = await self.bot.send_message(self.bot.get_channel(self.queueping), "{} - **Reminder**, you will be able to put up your recruitment post for {} in just under 4 hours.".format(" ".join(pinglist), self.queue["queue"]["1"]["TF"]))
                    self.queue["queue"]["1"]["ackpost"] = pingmsg.id
                # check if #1 is past ping time and #2 is within their ping window
                if len(self.queue["queue"]) > 1:
                    if now < self.queue["queue"]["2"]["posttime"] and now > self.queue["queue"]["2"]["pingtime"] and self.queue["queue"]["2"]["ackpost"] is None:
                        pingpeople = self.queue["TFs"][self.queue["queue"]["2"]["TF"]]
                        if len(pingpeople) > 2:
                            # only get 2 choices from the possible members to ping if there's more than 2
                            pingpeople = sample(pingpeople, k=2)
                        pinglist = []
                        for pingperson in pingpeople:
                            pinglist.append("<@" + pingperson + ">")
                        pingmsg = await self.bot.send_message(self.bot.get_channel(self.queueping), "{} - **Reminder**, you will be able to put up your recruitment post for {} in just under 4 hours.".format(" ".join(pinglist), self.queue["queue"]["2"]["TF"]))
                        self.queue["queue"]["2"]["ackpost"] = pingmsg.id
